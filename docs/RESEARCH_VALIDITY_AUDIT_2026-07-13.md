# Independent Research-Validity Audit — 2026-07-13
...
inference, and the manuscript is unusually explicit about oracle routing,
post-hoc repair, model-generated paraphrases, subscription-Codex limitations,
and pending human review.

It is **not currently safe to submit as a workshop paper**. The principal
empirical claims depend on a permissive result-matching metric, experiment
artifacts generated from an uncommitted working tree, and a benchmark-specific
deterministic compiler developed alongside the reference queries. Those issues
do not make the project fraudulent; they make the current percentages
development evidence rather than publication-grade estimates.
