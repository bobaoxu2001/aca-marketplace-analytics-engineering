...
| Plan count by county | How many standard-component plans are available in each county/service area? | `count(distinct plan_key)` | `fact_plan_availability` + `dim_geography` | State, `county_display_name`, county FIPS, issuer, metal level | Use `dim_geography.county_display_name` when enriched; otherwise fall back to raw CMS county value. |
| Issuer count by county | How competitive is a county by issuer presence? | `count(distinct issuer_key)` | `fact_plan_availability` + `dim_geography` | State, county display name, metal level | Join `dim_issuer` for `issuer_name` filters in BI tools. |
